...
- **Solução:** Migration `20260226_auth_features` (emailVerified, emailVerificationToken, emailVerificationExpiry, googleId, password nullable, status default PENDING). EmailModule global com Resend (sendVerificationEmail, sendSellerApprovedEmail, sendSellerRejectedEmail). Auth: register agora gera token + envia email (sem JWT), login checa emailVerified + status (PENDING seller = aguardando aprovação, BLOCKED = bloqueado). Novos endpoints: POST /auth/verify-email, POST /auth/resend-verification, POST /auth/google (google-auth-library). Admin: PATCH /admin/users/:id/status + filtro status no GET /admin/users. Web: register mostra tela "verifique email", login com tratamento de erros específicos + botão reenviar, Google OAuth via @react-oauth/google + GoogleOAuthProvider, página /verify-email. Admin UI: coluna status com badges coloridos, tabs de filtro (Todos/Pendentes/Ativos/Bloqueados), botões Aprovar/Rejeitar/Bloquear/Desbloquear com modal de confirmação.
- **Arquivos:** `prisma/schema.prisma`, `prisma/migrations/20260226_auth_features/`, `backend/src/modules/email/*`, `backend/src/modules/auth/auth.service.ts`, `backend/src/modules/auth/auth.controller.ts`, `backend/src/modules/auth/dto/verify-email.dto.ts`, `backend/src/modules/auth/dto/resend-verification.dto.ts`, `backend/src/modules/auth/dto/google-auth.dto.ts`, `backend/src/modules/auth/auth.service.spec.ts`, `backend/src/modules/admin/admin.service.ts`, `backend/src/modules/admin/admin.controller.ts`, `backend/src/modules/admin/dto/update-user-status.dto.ts`, `backend/src/app.module.ts`, `web/src/services/auth.service.ts`, `web/src/hooks/useAuth.ts`, `web/src/types/api.types.ts`, `web/src/app/providers.tsx`, `web/src/app/login/page.tsx`, `web/src/app/register/page.tsx`, `web/src/app/verify-email/page.tsx`, `admin/src/app/users/page.tsx`
