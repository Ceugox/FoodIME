...
### [2026-02-28] Funcionalidade "Esqueci minha senha"
- **Problema:** Usuários que esqueciam a senha não tinham como recuperar acesso — não existia fluxo de reset de senha no app web.
- **Solução:** Campos `passwordResetToken` (UUID v4, unique) e `passwordResetExpiry` (1h) adicionados ao User via Prisma. Backend: DTOs `ForgotPasswordDto` e `ResetPasswordDto`, métodos `forgotPassword()` e `resetPassword()` no AuthService, endpoints `POST /auth/forgot-password` (rate limit 3/min) e `POST /auth/reset-password` (rate limit 5/min) no AuthController. Email de reset via Resend (`sendPasswordResetEmail`). Segurança: mensagem genérica no forgot-password (sem enumerar emails), usuários Google-only ignorados (sem password), token uso único. Web: páginas `/forgot-password` e `/reset-password`, hooks `useForgotPassword` e `useResetPassword`, link "Esqueceu a senha?" na página de login.
- **Arquivos:** `backend/prisma/schema.prisma`, `backend/src/modules/auth/dto/forgot-password.dto.ts`, `backend/src/modules/auth/dto/reset-password.dto.ts`, `backend/src/modules/auth/auth.service.ts`, `backend/src/modules/auth/auth.controller.ts`, `backend/src/modules/email/email.service.ts`, `web/src/services/auth.service.ts`, `web/src/hooks/useAuth.ts`, `web/src/app/forgot-password/page.tsx`, `web/src/app/reset-password/page.tsx`, `web/src/app/login/page.tsx`

### [2026-02-26] Deploy Audit — 7 bugs críticos/altos corrigidos + Sentry fix
- **Problema:** JWT_SECRET hardcoded; email fictício no Pix; escalação de privilégio em /admin/users/:id; Sentry incompatível com Next.js 15; admin valores monetários divididos por 100.
- **Solução:** Middleware sem fallback hardcoded; payerEmail real; UpdateUserDto com @IsEnum; Sentry via instrumentation files; valores monetários corrigidos.
- **Arquivos:** `web/src/middleware.ts`, `backend/prisma/schema.prisma`, `backend/src/modules/payments/mercadopago.service.ts`, `backend/src/modules/admin/dto/update-user.dto.ts`, `web/instrumentation.ts`, `admin/instrumentation.ts`
...
- **Solução:** Migrations incrementais criadas (20260225_schema_sync + 20260225_store_hours); openTime/closeTime no Store; sistema Toast com @radix-ui/react-toast + hook useToast; página /orders/[id] com timeline visual + Web Share API; OrderCard clicável; polling dinâmico (5s quando pedido PAID) + vibração quando READY; OfflineBanner detecta navigator.onLine; RefreshButton em home/orders/seller; confirmação modal no logout; toasts em todas as mutations; Sentry config atualizada (sourcemaps.disable); dependências backend instaladas (@nestjs/throttler, @sentry/nestjs, joi, helmet, @types/multer); DEPLOY_GUIDE.md criado.
- **Arquivos:** `prisma/migrations/20260225_*`, `schema.prisma`, `web/hooks/useToast.ts`, `web/components/ui/toaster.tsx`, `web/components/common/OfflineBanner.tsx`, `web/components/common/RefreshButton.tsx`, `web/app/(buyer)/orders/[id]/page.tsx`, `web/app/(buyer)/orders/page.tsx`, `web/app/(buyer)/home/page.tsx`, `web/app/(buyer)/profile/page.tsx`, `web/app/(seller)/seller/store/page.tsx`, `web/app/(seller)/seller/orders/page.tsx`, `web/app/(seller)/seller/products/page.tsx`, `web/app/providers.tsx`, `web/hooks/useOrders.ts`, `web/types/models.types.ts`, `web/next.config.ts`, `admin/next.config.ts`, `backend/dto/create-store.dto.ts`, `docs/DEPLOY_GUIDE.md`
