    ...
    <div className="px-5 pt-4 pb-6 animate-slide-up">
      ...
      <div className="flex items-center justify-between mb-4">
        <div>
          ...
          <h1 className="text-xl font-serif text-text mt-0.5">
      ...
      {isLoading ? (
        <div className="space-y-4">
          ...
          <div className="h-40 bg-surface rounded-xl animate-shimmer" />
        </div>
      ) : isError ? (
        ...
        <>
          ...
          {/* Main revenue card */}
          <div className="bg-gradient-to-br from-surface to-surface-2 rounded-xl border border-border p-6 flex flex-col items-center mb-5">
            <p className="text-text-secondary text-sm">Receita</p>
            <p className="text-4xl font-serif text-text mt-1">{formatCurrency(revenue)}</p>
            <p className="text-text-muted text-sm mt-1">{orderCount} pedido{orderCount !== 1 ? 's' : ''}</p>
          </div>
                  ...
                  return (
                    <div key={i} className="flex-1 flex flex-col items-center gap-1">
                      <div className="w-full flex items-end justify-center" style={{ height: '80px' }}>
                        <div
                          className={`w-full rounded-t-md transition-all duration-500 ${isLast ? 'bg-accent' : 'bg-primary'}`}
                          style={{ height: `${pct}%` }}
          ...
          {/* Top product */}
          {metrics?.topProduct && (
            <div className="bg-warning/10 border border-warning/20 rounded-xl p-4 flex items-center gap-3 mb-5">
              ...
              <div className="flex-1">
                ...
                <p className="text-text font-serif text-sm">{metrics.topProduct.name}</p>
              </div>
              <span className="text-warning font-serif text-base">{metrics.topProduct.totalSold} un.</span>
            </div>
          )}

          {/* Transactions */}
          <p className="text-text font-serif text-base mb-3">Últimas transações</p>
          {!metrics?.transactions.length ? (
            <p className="text-center text-text-secondary text-sm py-8">Nenhuma transação ainda</p>
          ) : (
            <div className="space-y-2 mb-6">
              {metrics.transactions.slice(0, 10).map((tx, i) => (
                <div
                  key={tx.id}
                  className="bg-surface rounded-xl border border-border p-3 flex items-center justify-between hover:border-border-hover transition-all animate-slide-up"
                  style={{ animationDelay: `${i * 60}ms` }}
                >
                  <div>
                    <p className="text-text font-serif text-sm">#{tx.orderCode}</p>
          ...
          <button
            ...
            className="w-full h-11 rounded-xl border border-error/40 text-error text-sm font-semibold hover:bg-error/10 transition-all"
          >
            Sair da conta
          </button>
        </>
