...
function StoreCard({ store }: { store: Store }) {
  return (
    <Link href={`/store/${store.id}`}>
      <div className="flex items-center gap-0 bg-surface rounded-2xl border border-border overflow-hidden mb-3 hover:border-primary/50 transition-colors active:scale-[0.99]">
        {store.imageUrl ? (
        ...
        <div className="flex-1 p-3 min-w-0">
          <p className="font-bold text-text text-[15px] truncate">{store.name}</p>
  ...
  return (
    <div className="px-5 pt-4">
      ...
      <div className="flex items-center justify-between mb-5">
        <div>
          <h1 className="text-[26px] font-extrabold text-accent tracking-wide">FoodIME</h1>
      ...
      {isLoading ? (
        ...
        <div className="flex flex-col items-center justify-center py-20 gap-3">
          ...
          <p className="text-text-light text-sm">Volte mais tarde</p>
        </div>
      ) : (
