    ...
    return (
      <div className="px-5 pt-4">
        <div className="flex items-center gap-3 mb-6">
          ...
          <h1 className="text-lg font-bold text-text">Carrinho</h1>
        </div>
        <div className="flex flex-col items-center justify-center py-20 gap-4">
          <svg className="w-14 h-14 text-text-light" fill="none" viewBox="0 0 24 24" stroke="currentColor">
            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M2.25 3h1.386c.51 0 .955.343 1.087.835l.383 1.437M7.5 14.25a3 3 0 00-3 3h15.75m-12.75-3h11.218c1.121-2.3 2.1-4.684 2.924-7.138a60.114 60.114 0 00-16.536-1.84M7.5 14.25L5.106 5.272M6 20.25a.75.75 0 11-1.5 0 .75.75 0 011.5 0zm12.75 0a.75.75 0 11-1.5 0 .75.75 0 011.5 0z" />
          </svg>
          <p className="text-text-secondary font-semibold">Carrinho vazio</p>
          <button onClick={() => router.push('/home')} className="px-6 py-2.5 bg-primary text-white rounded-xl text-sm font-semibold">
            Explorar lojas
          </button>
        </div>
      </div>
    ...
    <div className="px-5 pt-4">
      ...
      <div className="space-y-3 mb-6">
        {items.map(({ product, quantity }) => (
          <div key={product.id} className="flex items-center gap-3 bg-surface rounded-2xl border border-border p-3">
            ...
            <div className="flex-1 min-w-0">
              ...
              <p className="text-accent text-sm font-bold mt-0.5">{formatCurrency(product.price)}</p>
            </div>
            <div className="flex items-center gap-1 bg-surface-2 rounded-xl px-1">
              <button onClick={() => updateQuantity(product.id, quantity - 1)} className="w-8 h-8 flex items-center justify-center text-accent font-bold">−</button>
      ...
      {/* Summary */}
      <div className="bg-surface rounded-2xl border border-border p-4 mb-4 space-y-2">
      ...
      <button
        ...
        className="w-full h-13 bg-primary hover:bg-primary-light disabled:opacity-60 text-white rounded-2xl font-bold text-base transition-colors"
      >
